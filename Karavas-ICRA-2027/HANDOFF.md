...
fixed, the sweep rerun and the paper rewritten. **Read this first.** It is the
full briefing: what the work is, what was decided and why, what was found, what
to run, and what is still open.
...
- **AI-generated content must be disclosed in the acknowledgments**, naming the
  ...
  non-refundable. **Status:** §IV–VI, the abstract and the contributions
  paragraph were first drafted with an AI assistant against the measured
  outputs. The author's stated plan (2026-09-12) is to **rewrite the paper in
  their own words**, which reduces the remaining contribution to editorial
  assistance and removes the need to disclose. That only holds if the rewrite is
  real rather than a paraphrase pass — confirm before submitting.
...
**The whole paper is drafted and compiles to exactly 8 pages** with no undefined
references and no author or subject identifiers in the PDF. §I–§III were revised
in the earlier Windows session; the abstract, contributions paragraph, §III-C
and §IV–§VI were written against the measured outputs and then updated twice as
the pipeline fixes changed the numbers.
...
**Citations.** `References.bib` now holds **14 added references**, every DOI
verified against Crossref (Bai2018 against arXiv). Keys by gap: CNN/TCN —
`Molinaro2022`, `Li2018`, `Bai2018`; LSTM/GRU gait — `Zaroug2020`, `Zaroug2021`,
`Ren2022`; hybrids/attention — `Zhu2021`, `Lu2022`, `Aksan2021`; phase variables
— `Holgate2009`, `Rezazadeh2019`, `Embry2021`, `Kang2021`, `Medrano2023`.
...
**Still open:**
- The four `TODO(refs)` markers in §I are still in place — the verified entries
  exist in the `.bib` but have not been cited into the text yet.
- Restore the real author block and add an ELEPAP acknowledgement for
  camera-ready (there is currently **no acknowledgements section at all**).
...
- The title is still the old survey-ish framing and no longer matches the
  argument.

## 8. Traps

- **Page budget — already spent.** Exactly 8 pages. To get there, the
  architecture figure, the nine-panel per-channel figure and two Fig. 2 panels
  were cut, prose was tightened several times, and `figure*` widths reduced.
  **Citing the 14 new references will push it over** — 11 references currently
  take 31 lines, so budget ~25 more. The likely next cut is the per-joint table
  (Table II) or the per-phase table, each of which the prose can carry.
- **Do not invent citations.** The 14 added entries were each verified against
  ...
  references would be a serious error.
