#!/usr/bin/env python3
"""
phase_variable_lstm_all_planes.py
Phase Variable LSTM — all 18 angle channels (all 3 planes per joint).

Extends phase_variable_lstm.py from sagittal-only (6 channels) to
...
(same as the 6-channel version). All other logic is identical.
"""
...
# Phase variable is still derived from sagittal hip angles
...
def plot_phase_abs_error_18ch(gt_strides, pred_strides, title="Mean |Pred−GT| vs gait phase"):
    ...
    abs_err = np.abs(pred_strides - gt_strides).mean(axis=0)   # (S, 18)
...
def rollout_recursive_with_measured_pv(
    ...
    """
    Recursive next-tick rollout.
    PV is treated as exogenous (read from the measured stream).
    Predicted angles are fed back as the angle part of the next window step.
    """
...
def main():
    ...
    if not os.path.exists(TYPICAL_FILE):
        raise FileNotFoundError(
            ...
            "If 'Left Foot Off' / 'Right Foot Off' are absent from _all_columns.xlsx, "
            "use 'Data_Normal/randomized_data_healthy.xlsx' instead."
        )
    ...
    if missing:
        raise KeyError(
            f"Typical file is missing columns: {sorted(missing)}\n"
            "Try using 'Data_Normal/randomized_data_healthy.xlsx' for TYPICAL_FILE."
        )
    ...
    best_path = os.path.join(SAVE_DIR, "PV_lstm_best_rollout_dtw_18ch.keras")
    ...
    final_path = os.path.join(SAVE_DIR, "PV_lstm_next_tick_18ch_final.keras")
    ...
    plot_pred_vs_gt_18ch(pred_deg, gt_deg, title=f"PV LSTM next-tick (WINDOW={WINDOW})")
    ...
    pd.DataFrame(pred_out, columns=out_cols).to_excel(
        os.path.join(PRED_DIR, "rolling_pred_next_tick_with_pv_lstm_18ch.xlsx"), index=False
    )
    pd.DataFrame(gt_out, columns=out_cols).to_excel(
        os.path.join(PRED_DIR, "rolling_gt_next_tick_with_pv_lstm_18ch.xlsx"), index=False
    )
    ...
    for metric, label in [("loss", "MSE Loss"), ("mae", "MAE")]:
        ...
        plt.title(f"PV LSTM 18ch — {label}")
        plt.xlabel("Epoch"); plt.ylabel(label)
        plt.grid(True); plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(PLOT_DIR, f"pv_lstm_18ch_{metric}.png"), dpi=200)
        plt.show()
